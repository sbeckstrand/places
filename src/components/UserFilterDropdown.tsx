...
export type UserFilterOption = { id: string; label: string };
...
export default function UserFilterDropdown({
  ...
  const label =
    selected.size === options.length
      ...
      : selected.size === 0
        ? "No one"
        : `${selected.size} of ${options.length} people`;
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute left-0 top-full z-10 mt-1 flex w-48 flex-col gap-0.5 rounded-md border border-neutral-200 bg-white p-1.5 shadow-lg dark:border-neutral-700 dark:bg-neutral-900">
          {options.map((o) => (
            <label
              key={o.id}
